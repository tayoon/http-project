...
int main(int argc, char **argv) {
  ...
  //
  //  |Length(24bit)|Type(8bit)|Flags(8bit)|Reserve(1bit)|Stream
  //  Identifier(31bit)| |Frame Payload(Lengthバイト分)|
  //
  //
  // [フレームのタイプ]
  //
  ...
  //------------------------------------------------------------
  const char settingframe[BINARY_FRAME_LENGTH] = {0x00, 0x00, 0x00, 0x04, 0x00,
                                                  0x00, 0x00, 0x00, 0x00};
  ...
  // 5バイト目にフラグ0x01を立てます。
  //------------------------------------------------------------
  const char settingframeAck[BINARY_FRAME_LENGTH] = {
  ...
  // ストリームIDは「0x01」を使います.
  //
  // ここまででヘッダフレームは「ペイロードの長さ(3バイト), 0x01, 0x05, 0x00,
  // 0x00, 0x00, 0x01」になります.
  //
  //
  // ●HTTP1.1でのセマンティクス
  // 　　"GET / HTTP1/1"
  ...
  //
  // |0|0|0|0|      0|   //
  // 最初の4ビットは圧縮に関する情報、次の4ビットはヘッダテーブルのインデクス.(今回は圧縮しないのですべて0)
  // |0|            7|   //
  // 最初の1bitは圧縮に関する情報(今回は0)、次の7bitはフィールドの長さ
  // |:method|           // フィールドをそのままASCIIのオクテットで書く。
  // |0|            3|   //
  // 最初の1bitは圧縮に関する情報(今回は0)、次の7bitはフィールドの長さ |GET| //
  // 値をそのままASCIIのオクテットで書く。
  //
  ...
  const char headersframe[69] = {
      ...
      0x00,                                                  // 圧縮情報
      0x0a, 0x3a, 0x61, 0x75, 0x74, 0x68, 0x6f, 0x72, 0x69,
      0x74, 0x79,  // 10 :authority
      0x0b, 0x6e, 0x67, 0x68, 0x74, 0x74, 0x70, 0x32, 0x2e,
      0x6f, 0x72, 0x67  // 11 nghttp2.org
  };
  ...
  //------------------------------------------------------------
  const char goawayframe[17] = {0x00, 0x00, 0x00, 0x07, 0x00, 0x00,
